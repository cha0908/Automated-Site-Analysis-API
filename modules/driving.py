...
def _add_route_arrow(ax, gdf_route, color, position=1.0, zorder=20):
    ...
    if pos <= 0.0:
        ...
        head_x, head_y = x0, y0
        back_t = 0.15
        tail_x = x0 + back_t * (x1 - x0); tail_y = y0 + back_t * (y1 - y0)
    elif pos >= 1.0:
        (x0, y0) = coords[-2]; (x1, y1) = coords[-1]
        head_x, head_y = x1, y1
        back_t = 0.15
        tail_x = x1 - back_t * (x1 - x0); tail_y = y1 - back_t * (y1 - y0)
    else:
        dist_so_far = 0.0; seg_index = 0
        for i, seg_len in enumerate(seg_lengths):
            if dist_so_far + seg_len >= target_dist:
                seg_index = i; break
            dist_so_far += seg_len
        (x0, y0) = coords[seg_index]; (x1, y1) = coords[seg_index + 1]
        seg_len = seg_lengths[seg_index]
        if seg_len == 0: return
        local_t = (target_dist - dist_so_far) / seg_len
        head_x = x0 + local_t * (x1 - x0); head_y = y0 + local_t * (y1 - y0)
        back_t = max(0.0, local_t - 0.15)
        tail_x = x0 + back_t * (x1 - x0); tail_y = y0 + back_t * (y1 - y0)
    if head_x == tail_x and head_y == tail_y:
        return
    ax.annotate("", xy=(head_x, head_y), xytext=(tail_x, tail_y),
                arrowprops=dict(arrowstyle="-|>", color=color, lw=3, mutation_scale=26),
                zorder=zorder)
...
def _draw_tofrom(ax, x, y, name, zorder=18):
    ax.text(x, y, f"TO/FROM\n{name}", fontsize=8.5, weight="bold", color="black",
            ha="center", va="center", zorder=zorder, multialignment="center",
            bbox=dict(facecolor="white", edgecolor="black",
                      linewidth=1.5, pad=4, boxstyle="square,pad=0.4"))


def _north_arrow(ax, xlim, ylim, extent):
    x = xlim[0] + 0.965*(xlim[1]-xlim[0]); y = ylim[0] + 0.940*(ylim[1]-ylim[0])
    L = extent * 0.055
    ax.annotate("", xy=(x, y+L*0.6), xytext=(x, y-L*0.6),
                arrowprops=dict(arrowstyle="-|>", color="black", lw=2, mutation_scale=16), zorder=25)
    ax.text(x, y+L*0.85, "N", fontsize=13, weight="bold", color="black",
...
def generate_driving(data_type: str, value: str,
                ...
                try:
                    ...
                    if len(osm):
                        ...
                        if candidate.centroid.distance(site_pt_3857) < dist * 2:
                            site_poly = candidate; break
                except Exception:
                    pass
        if site_poly is None:
            for tags in [{"landuse": True}, {"amenity": True}]:
                try:
                    osm = ox.features_from_point((lat, lon), dist=100, tags=tags).to_crs(3857)
                    polys = osm[osm.geometry.geom_type.isin(["Polygon","MultiPolygon"])]
                    if len(polys):
                        polys = polys.copy()
                        polys["_d"] = polys.geometry.centroid.distance(site_pt_3857)
                        site_poly = polys.sort_values("_d").geometry.iloc[0]; break
                except Exception:
                    pass
        if site_poly is None:
            site_poly = site_pt_3857.buffer(80)
    ...
    cx.add_basemap(ax, source=cx.providers.CartoDB.PositronNoLabels, zoom=zoom, alpha=0.55)
    ...
    for ring_r, ring_lbl in cfg["rings"]:
        gpd.GeoSeries([centroid.buffer(ring_r)], crs=3857).boundary.plot(
            ax=ax, color="#e6b800", linewidth=2, linestyle=(0, (4, 3)), zorder=5)
        ax.text(cx_val + ring_r + 30, cy_val, ring_lbl, fontsize=10, color="black",
                weight="bold", ha="left", va="center", zorder=10, clip_on=False)
    ...
    for _, station in stations.iterrows():
        ...
        if egress_gdf is not None and ingress_gdf is not None:
            if len(egress_gdf) == len(ingress_gdf):
                overlap = all(e.equals(i) for e, i in zip(egress_gdf.geometry, ingress_gdf.geometry))

        if overlap:
            ingress_gdf.plot(ax=ax, linewidth=3.5, color=INGRESS_COLOR, alpha=0.92, zorder=8)
            ...
            if egress_gdf is not None:
                egress_gdf.plot(ax=ax, linewidth=3.5, color=EGRESS_COLOR, alpha=0.92, zorder=8)
                _add_route_arrow(ax, egress_gdf, EGRESS_COLOR, position=0.9, zorder=20)
            if ingress_gdf is not None:
                ingress_gdf.plot(ax=ax, linewidth=3.5, color=INGRESS_COLOR, alpha=0.92, zorder=9)
                _add_route_arrow(ax, ingress_gdf, INGRESS_COLOR, position=1.0, zorder=21)
        ...
        gpd.GeoSeries([st_geom], crs=3857).plot(
            ax=ax, facecolor="#5dade2", edgecolor="#2e86c1", linewidth=2, alpha=0.5, zorder=23)
        _add_mtr_icon(ax, st_cen.x, st_cen.y, size=0.035, zorder=24)
        ax.text(st_cen.x, st_cen.y + MAP_EXTENT*0.060, st_name,
                fontsize=10, weight="bold", color="black", ha="center", va="bottom", zorder=25)

    site_gdf.plot(ax=ax, facecolor="red", edgecolor="none", alpha=0.85, zorder=11)
    ax.text(cx_val, cy_val - MAP_EXTENT*0.06, "SITE",
            color="black", weight="bold", ha="center", fontsize=11, zorder=12)

    ax.legend(
        handles=[
            mlines.Line2D([], [], color=INGRESS_COLOR, linewidth=3, marker=">",
                          markersize=9, markerfacecolor=INGRESS_COLOR, label="INGRESS ROUTING"),
            mlines.Line2D([], [], color=EGRESS_COLOR, linewidth=3, marker=">",
                          markersize=9, markerfacecolor=EGRESS_COLOR, label="EGRESS ROUTING"),
        ],
        loc="lower right", frameon=True, facecolor="white", edgecolor="black",
        fontsize=10, title="LEGEND", title_fontsize=10, framealpha=1.0
    ...
    ax.set_axis_off()
    ax.set_title(
        f"SITE ANALYSIS – Driving Distance ({max_drive_minutes} min) – {data_type} {value}",
